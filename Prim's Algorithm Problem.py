...
class Graph:
    ...
    def prims(self):#we neeed to store three things here parent,visited and weight
        visited = [False for i in range(self.nVertices)]#initially it will be false for all 
        parent = [-1 for i in range(self.nVertices)]#initially it will be -1 for all edge
        weight = [sys.maxsize for i in range(self.nVertices)]
        weight[0] = 0
        
        for i in range(self.nVertices - 1):#we will do process for (n-1) times
            min_vertex = self.__getMinVertex(visited, weight)#minvertex is one which is not visited and have minimum weight
